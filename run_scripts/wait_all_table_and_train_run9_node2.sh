...
VAL_SEQUENCES=(box_seq_11 floor_seq_02 table_seq_05 wall_seq_02)
...
log() {
  printf '[%s] %s\n' "$(date -Is)" "$*" | tee -a "$LOG_DIR/all_table_pipeline.log"
}
...
ax.set_xlabel("epoch")
ax.set_ylabel("RMSE")
ax.set_title("Run 9 RMSE over epochs")
...
ax.set_ylabel("RMSE")
ax.set_title("Run 9 checkpoint RMSE by sequence")
...
main() {
  cd "$ROOT"
  log "all-table post-sweep training pipeline started"
  log "run dir: $RUN_DIR"
  ...
  log "generating GT-only document with all table sequences"
  "$PYTHON" EvMotionSeg/tools/make_evimo_train_gt_document.py \
    --sweep-root "$SWEEP_ROOT" \
    --skip-training-report \
    --output "$SWEEP_ROOT/evimo_train_gt_document_with_all_tables.md" \
  ...
  log "starting run 9: same config as run 8, all table sequences added"
  ...
  log "generating run 9 GT/training document"
  "$PYTHON" EvMotionSeg/tools/make_evimo_train_gt_document.py \
    --sweep-root "$SWEEP_ROOT" \
