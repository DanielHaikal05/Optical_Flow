...
subset_base="${ZURICH00A_CAND_SUBSET_BASE:-EvMotionSeg/data/dsec_zurich_city_00_a_target_car10_dense_sweep_20260813_111455/target_car10_dense_base}"
run_group="${ZURICH00A_CAND_GROUP:-EvMotionSeg/data/dsec_zurich_city_00_a_target_car10_data_candidate_calib_$(date +%Y%m%d_%H%M%S)}"
build_dir="${ZURICH00A_CAND_BUILD_DIR:-/tmp/dsec_evmotionseg_zurich00a_data_candidate_calib_build}"

mkdir -p "${run_group}/logs"
printf "%s\n" "${run_group}" > EvMotionSeg/data/dsec_zurich_city_00_a_target_car10_data_candidate_calib_latest_path.txt
...
log "Building standalone binary for data/candidate calibration."
...
# Keep the original smooth/label scale, but give the data term enough weight to compete
# with graph-cut smoothness. The richer candidate pool tests whether rare car-flow models
# were simply not being seeded.
run_one 10 7000 90000 1 background_fit 3.0 0.10 constant 25 0.05 18 1.25 64 32 0.001 0.50 legacy
run_one 30 7000 90000 1 background_fit 3.0 0.10 constant 25 0.05 18 1.25 64 32 0.001 0.50 legacy
run_one 60 7000 90000 1 background_fit 3.0 0.10 constant 25 0.05 18 1.25 64 32 0.001 0.50 legacy
run_one 30 12000 90000 1 background_fit 3.0 0.10 constant 25 0.05 18 1.25 64 32 0.001 0.50 legacy

log "Data/candidate calibration complete: ${score_csv}"
