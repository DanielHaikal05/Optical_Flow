...
run_group="${ZURICH00A_DENSE_REFINE_GROUP:-EvMotionSeg/data/dsec_zurich_city_00_a_target_car10_dense_refine_$(date +%Y%m%d_%H%M%S)}"
build_dir="${ZURICH00A_DENSE_REFINE_BUILD_DIR:-/tmp/dsec_evmotionseg_zurich00a_dense_refine_build}"

mkdir -p "${run_group}/logs"
status_log="${run_group}/logs/status.log"
score_csv="${run_group}/target_scores.csv"
printf "%s\n" "${run_group}" > EvMotionSeg/data/dsec_zurich_city_00_a_target_car10_dense_refine_latest_path.txt
printf "run_dir,smooth,label,downsample,background_mode,bg_error_ratio,bg_min_fraction,smoothness_mode,sigma,min_weight,cand_angle,cand_ratio,initial_candidates,tracked_bonus,track_min,track_max,car_recall,noncar_fpr,car_precision,raw_best_recall,raw_best_purity,raw_label_mean,raw_label_max,nonzero_imo_frames,frames_over_5pct\n" > "${score_csv}"

log() {
  printf '[%(%Y-%m-%d %H:%M:%S)T] %s\n' -1 "$*" | tee -a "${status_log}"
}

log "Building standalone binary for dense-base refinement."
...
run_one() {
  ...
  local bg_error_ratio="$5"
  local bg_min_fraction="$6"
  local smooth_mode="$7"
  local sigma="$8"
  local min_weight="$9"
  local cand_angle="${10}"
  local cand_ratio="${11}"
  local initial_candidates="${12}"
  local tracked_bonus="${13}"
  local track_min="${14}"
  local track_max="${15}"
  local name="s${smooth}_l${label}_d${downsample}_${background_mode}_bgr${bg_error_ratio}_bgm${bg_min_fraction}_${smooth_mode}_sig${sigma}_mw${min_weight}_ang${cand_angle}_r${cand_ratio}_ic${initial_candidates}_tb${tracked_bonus}_tm${track_min}"
  ...
  "${python_bin}" - "${out}" "${score_csv}" "${smooth}" "${label}" "${downsample}" "${background_mode}" "${bg_error_ratio}" "${bg_min_fraction}" "${smooth_mode}" "${sigma}" "${min_weight}" "${cand_angle}" "${cand_ratio}" "${initial_candidates}" "${tracked_bonus}" "${track_min}" "${track_max}" <<'PY'
import csv
import json
import sys
...
# Dense-base refinement: first isolate the effect of multi-background selection,
# then test a narrow high-regularization band to avoid scene-region clutter.
run_one 7000 90000 1 background_fit_multi 1.5 0.04 constant 25 0.05 45 2.0 12 6 0.02 0.20
run_one 9000 120000 1 background_fit_multi 1.5 0.04 constant 25 0.05 45 2.0 12 6 0.02 0.20
run_one 11000 150000 1 background_fit_multi 1.5 0.04 constant 25 0.05 45 2.0 12 6 0.02 0.20
run_one 8000 120000 1 background_fit_multi 1.2 0.04 flow_edge 20 0.20 35 1.7 18 8 0.01 0.25
run_one 10000 150000 1 background_fit_multi 1.2 0.04 flow_edge 20 0.20 35 1.7 18 8 0.01 0.25

log "Dense-base refinement complete. Scores: ${score_csv}"
